...
document.addEventListener("DOMContentLoaded", () => {
  ...
  preloaderTl.to(preloaderElement, {
    opacity: 0,
    duration: 1,
    onComplete: () => {
  ...
  const portfolioGridItems = document.querySelectorAll(
    ".portfolio-grid .portfolio-item"
  );
  
  // Cache the class check for performance
  const portfolioGridClass = document.querySelector(".portfolio-grid");
  ...
  portfolioGridItems.forEach((item) => {
    ...
    item.addEventListener("mouseenter", () => {
      ...
      if (portfolioGridClass.classList.contains("list-view")) {
        floatImg.src = imgSrc;
  ...
  if (currentHash) {
    const hashBtn = filterBtnsArray.find(
    ...
    const allBtn = filterBtnsArray.find(
